...
class Obstacle():
    def __init__(self, x:float, y:float, radius:float=0.25):
        self.x = x
        self.y = y
        self.radius = radius
...
#location and if so, stores the lowest cost found for the current location as the node.
           ...
           for i in range(0,len(obstacle_reallist)):
             distance = numpy.sqrt(((current_node.x - obstacle_reallist[i].x)**2+(current_node.y - obstacle_reallist[i].y)**2))
             if (robot_radius + obstacle_reallist[0].radius) > distance:
...
#   Storing the given obstacles into the Obstacle class with the given diameter.
    ...
    obstacle_reallist.append(obstacle)
